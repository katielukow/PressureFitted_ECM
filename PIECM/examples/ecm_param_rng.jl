...
function ecm_err_range(data, Q, ocv, soc)
    xrng = 1e-5:0.0001:0.010
    yrng = 4000:50:6000
    ...
    for i in 1:length(xrng)
        for j in 1:length(yrng)
            r1 = xrng[i]
            c1 = yrng[j]
            v = ecm_discrete([r1, c1, 0.004], 1, df."Current(A)", df."Test_Time(s)", 0.999, Q, ocv, soc)
...
z0 = ecm_err_range(P0kpa, 5.5, ocv2, 0.5)
z50 = ecm_err_range(mbpf50kpa, 5.5, ocv2, 0.5)
z100 = ecm_err_range(mbpf100kpa, 5.5, ocv2, 0.5)

min0 = argmin(z0[2:end, 2:end])
min50 = argmin(z50[2:end, 2:end])
min100 = argmin(z100[2:end, 2:end])

trace1 = contour(z=z50[2:end, 2:end].-z0[2:end, 2:end], x=z0[2:end, 1], y=z0[1, 2:end])
layout = Layout(
    title="Error Surface",
...
# plot(surface(z=z0[2:end, 2:end], x=z0[2:end, 1], y=z0[1, 2:end]))
# plot(surface(z=z50[2:end, 2:end], x=z50[2:end, 1], y=z50[1, 2:end]))
# plot(surface(z=z100[2:end, 2:end], x=z100[2:end, 1], y=z100[1, 2:end]))
